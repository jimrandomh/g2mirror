# g2mirror protocol

This document is for implementers of device drivers (e.g. the Even Realities
G2 smart-glasses driver) that display g2mirror terminal sessions. Protocol
version: **1**.

## Architecture

```
┌──────────────┐  websocket   ┌─────────────────┐  unix socket   ┌───────────────┐
│ device driver│◄────────────►│ g2mirror-server │◄──────────────►│ g2mirror      │
│ (you)        │   (JSON)     │ (gateway)       │  (NDJSON)      │ (one per      │
└──────────────┘              └─────────────────┘                │  wrapped app) │
                                                                 └───────────────┘
```

- Each `g2mirror <command>` invocation wraps one CLI app in a pty and exposes
  a **session socket** at `~/.g2mirror/<pid>-<sanitized-cwd>` (newline-
  delimited JSON). Several clients may be connected — and viewing — at once.
- `g2mirror-server` is a websocket gateway: it authenticates the device,
  lists live sessions, and relays messages between the device and one
  session at a time per websocket connection.
- A driver normally talks only to the server. The session protocol is
  documented too because most messages are relayed through verbatim.
- `g2mirror-view` is a reference client for human viewers (a TUI that
  mirrors a session into a regular terminal); it speaks exactly the
  protocol described here.

Transport security is deliberately out of scope: the server listens on the
address in `~/.g2mirror/config.json` (loopback by default) and is intended to
be reached over tailscale or an ssh tunnel. Run `g2mirror-server
--init-config` once to generate the config and its first auth token, and
`g2mirror-server --add-token <name> [--writable]` for additional viewers
(tokens are printed once; only SHA-256 hashes are stored). Each token has a
name, its own `readonly` flag (default **true**; the `--init-config` token
is writable), an optional place in the `size_precedence` order (below),
and an optional `filter` — regex rules on a terminal's working directory
and/or window title restricting which terminals the token may see (see
README). To a driver, a filter is invisible except that: `list` omits
non-matching terminals, `connect` to one yields an `error`, non-matching
terminals produce no bell/title notifications, and an attached terminal
that stops matching (its title changed) is force-disconnected with
`{"type": "disconnected", "reason": "terminal no longer matches your
token's filter"}`.

## Framing

- **Websocket (device ↔ server):** each message is one JSON object in one
  websocket **text** frame.
- **Session socket (server ↔ wrapper):** each message is one JSON object on
  one `\n`-terminated line.

Every message has a string `type` field. Unknown fields must be ignored.
On either link, a message whose `type` the receiver doesn't recognize is an
error only where this document says so; drivers should tolerate and ignore
unknown message types for forward compatibility.

## Device ↔ server

### Handshake

The device's **first** message must be `init`:

```json
{"type": "init", "version": 1, "auth_token": "<token>",
 "device": "Jim's G2 glasses", "width": 96, "height": 24}
```

- `version` — protocol version; the server rejects mismatches.
- `auth_token` — any token from the server config (`--init-config` /
  `--add-token`), in the clear (the transport is assumed private).
- `device` — free-text description, shown to humans.
- `width`/`height` — the device's terminal size **in character cells**
  (columns/rows). This depends on the device font size. It is fixed for the
  lifetime of the websocket connection; to change it, disconnect and
  reconnect with new values. Note that with multiple simultaneous viewers
  the stream is not necessarily at your size — see "Snapshot dimensions
  and size precedence" below.

Success reply:

```json
{"type": "init", "version": 1, "readonly": false}
```

`readonly` reflects the authenticated token's flag in the server config:
when true, the server rejects every `input` message itself. A session can
additionally be read-only via its wrapper's `--readonly` flag, reported in
its `connect` message — input works only when neither is set, so a driver
should offer input only when both flags are false.

Failure reply (`error`), then the server closes the connection:

```json
{"type": "error", "message": "authentication failed"}
```

### Server-scoped requests

**`{"type": "list"}`** — enumerate live sessions:

```json
{"type": "sessions", "sessions": [
  {"socket": "84210-_Users_jim_repos_myproj", "pid": 84210,
   "cwd_hint": "_Users_jim_repos_myproj", "last_bell_at": 1782264921042,
   "title": "vim notes.md"}
]}
```

`socket` is an opaque handle to pass to `connect`. `cwd_hint` is the
sanitized working directory — display it to help the user pick a session.
`last_bell_at` is when that terminal's bell last rang (unix epoch ms), and
`title` is the window title the app last set (xterm OSC 0/2, e.g. what
would appear in a terminal emulator's title bar — often the running command
or an agent's status line); until the app sets one it is the wrapper's
`--title` value, if that was given. Either is `null` if the event hasn't happened
since the server began monitoring the terminal (the server monitors every
terminal from the moment it discovers its socket, whether or not any device
is attached; it does not know about bells or titles from before then or
while the server was down).

**`{"type": "connect", "socket": "84210-_Users_jim_repos_myproj"}`** —
attach to a session. The server dials the session socket and sends it a
session-protocol `init` derived from your websocket `init` (same `device`,
`width`, `height`). On failure you get an `error` message. On success the
next message you receive is the session's `connect` (below).

**`{"type": "disconnect"}`** — detach from the session. Reply:
`{"type": "disconnected", "reason": "requested"}`. You also receive a
`disconnected` message if the session ends or errors out
(`"reason": "session closed"`).

### Unsolicited bell and title notifications

Whenever any monitored terminal rings its bell — attached or not, viewed or
not — every authenticated device receives:

```json
{"type": "bell", "socket": "84210-_Users_jim_repos_myproj",
 "last_bell_at": 1782264921042}
```

Bell notifications for one terminal arrive at most every 3 seconds: the
first bell is reported immediately, and bells rung inside the window are
coalesced into one message when the window expires, so `last_bell_at`
catches up to the latest bell. Use this to surface "an agent wants
attention" cues on the glasses; ring the bell in the wrapped program
(`printf '\a'`) to trigger it.

Likewise, whenever a terminal app sets its window title (xterm OSC 0 or
OSC 2, BEL- or ST-terminated), every authenticated device receives:

```json
{"type": "title", "socket": "84210-_Users_jim_repos_myproj",
 "title": "vim notes.md"}
```

Title notifications are sent on change only (repeated sets of the same
title are suppressed) and are not debounced. An empty string is a valid
title (apps use it to clear the title bar).

### Relay

While attached to a session:

- Every message the session sends is forwarded to you verbatim.
- Every message you send whose `type` is not `init`, `list`, `connect`, or
  `disconnect` is forwarded to the session verbatim (`view`, `unview`).

Sending a non-server-scoped message while not attached yields an `error`.

## Session protocol (wrapper ↔ its client)

Once attached, these are the messages you exchange (through the relay).

A connection's first message determines its role: `init` makes it a
**viewer** (several may be connected and viewing at once), while
`{"type": "monitor", "version": 1}`
makes it a **monitor** — a role used by g2mirror-server to receive
`{"type": "bell", "at": <unix ms>}` notifications (debounced to one per 3s,
trailing bell reported when the window expires). A monitor does not count as
a viewer: it cannot send `view`/`unview` and does not block a device from
attaching. Drivers talk through the server and never need the monitor role
themselves; it is documented here for completeness.

The session `init` carries two extra fields the server fills in from its
config: `size_rank` (the position of the connection's token in
`size_precedence`) and `host_size_rank` (the position of `"host"` in the
same list); lower ranks win the app's size. Clients connecting to the
session socket directly may omit both, which gives them rank 0 and keeps
the host below every viewer.

### Snapshot dimensions and size precedence

When several clients view one terminal at once, the wrapped app is resized
to the dimensions of the **best-ranked** viewing client — or left at the
host terminal's size when `"host"` outranks them all — and every viewing
client receives the same snapshot/output stream at that size. The
`snapshot` message states the stream's `width` and `height`: feed your
emulator at *those* dimensions, not necessarily the ones you declared in
`init`, and crop or scroll as you see fit if your screen is smaller
(g2mirror-view crops bottom-left, like the wrapper's host rendering).

Whenever the stream's dimensions change (a better-ranked viewer attached,
detached, or the winning host terminal was resized), every viewing client
receives a **fresh unsolicited `snapshot`**: reset your emulator to the new
dimensions, apply it, and continue with the `output` stream. A driver that
always ranks first (the usual glasses setup) only ever sees snapshots at
its own size, but should still be prepared for an unsolicited one.

### From the session

**`connect`** — sent as soon as the wrapper accepts the connection:

```json
{"type": "connect", "version": 1, "pid": 84210, "command": "vim notes.md",
 "cwd": "/Users/jim/repos/myproj", "host_width": 143, "host_height": 40,
 "readonly": false, "history": {"next": 1523, "oldest": 0}}
```

`host_width`/`host_height` are the size of the terminal the wrapper is
running in (cells), for display purposes. `readonly` is true when the
wrapper was started with `--readonly` and will reject `input`. `history`
is the scrollback archive extent (see "Scrollback history" below).

**`snapshot`** — the immediate answer to `view`, and re-sent unsolicited
whenever the stream's dimensions change (see "Snapshot dimensions and size
precedence" above):

```json
{"type": "snapshot", "data": "<base64>", "width": 96, "height": 24,
 "history_next": 1523}
```

`data` is base64 of raw terminal bytes (VT100/xterm escape sequences).
Feed the decoded bytes to a terminal emulator of exactly `width` ×
`height` cells (the best-ranked viewer's size — usually yours, when your
token leads `size_precedence`), **reset/cleared first** — the snapshot
begins with a clear and repaints everything, including cursor
position/visibility and input modes. Wrappers from before these fields
existed omit them; such a wrapper supports one viewer and sizes the stream
to the `width`/`height` you declared in init, so fall back to those. Its content is the bottom-left crop
of the app's screen at the moment the view (re)started (the app itself
repaints at the new dimensions a moment later; that arrives as `output`).

**`output`** — incremental terminal bytes while viewing, same base64
encoding. Apply them to the same emulator, in order, after the snapshot.
The stream is a diff against the emulator's current state, so messages must
not be dropped or reordered; if you lose state, send `view` again for a
fresh snapshot.

**`exit`** — the wrapped app exited; the wrapper closes the socket (you'll
then get `disconnected` from the server):

```json
{"type": "exit", "status": 0}
```

`status` is the exit code, or `null` if the app died to a signal.

**`title`** — the app set the window title:

```json
{"type": "title", "title": "vim notes.md"}
```

Sent to both the viewer and the monitor whenever the title changes, and
once on attach if the app has already set one. While viewing, the wrapper
also re-emits the title sequence on its host terminal, so the host title
bar stays in sync too.

**`error`** — protocol violation report; the wrapper closes the connection
after sending it.

### To the session

**`init`** — first message; the server sends this for you when relaying.

**`{"type": "view"}`** — start viewing. Effects, in order:

1. The wrapped app's pty is resized to the best-ranked viewing client's
   `width`×`height` — yours, if you rank best (the app gets SIGWINCH and
   will typically repaint itself).
2. You immediately receive `snapshot` (as does every other viewing client).
3. Terminal output streams to you as `output` messages (usually starting
   with the app's SIGWINCH repaint) until `unview` or disconnect.

Sending `view` while already viewing is allowed and simply re-snapshots
(use it to resynchronize).

**`{"type": "unview"}`** — stop viewing: output streaming to you stops and
the app is resized for whoever is still viewing (back to the host terminal
size when nobody is). Idempotent. Use this whenever the glasses UI
navigates away from the terminal so the app returns to full size for the
person at the keyboard.

**`{"type": "input", "data": "<base64>"}`** — write bytes to the app's
terminal, exactly as a terminal emulator would send them for keystrokes.
For plain text (e.g. voice-to-text) this is just the UTF-8 text, typically
followed by `\r` (0x0D — what the Enter key sends) to submit it. Accepted
whether or not you are currently viewing. If the session or the server is
read-only, the rejecting side replies with an `error` message but keeps the
connection open.

### Scrollback history

The wrapper archives every line that scrolls off the (primary) screen —
including output from before any client connected — as an append-only
sequence with stable line indices, retained up to a cap (10,000 lines by
default; `--scrollback` changes it). Alternate-screen output (vim, htop)
produces no history, matching xterm.

**The client-side scrollback contract:** your emulator witnesses every line
that scrolls off while you are viewing, so retain those locally and treat
the archive as the source for what you *didn't* see. The `history_next`
value in each `snapshot` is the splice point: lines with index <
`history_next` are fetchable from the archive; lines your emulator sees
scroll off after the snapshot continue from that index. On re-`view`, the
new snapshot's `history_next` tells you how much appeared in between.

Fetch lazily, paging backwards from the splice point:

```json
{"type": "history", "before": 1523, "limit": 200}
```

```json
{"type": "history_lines", "start": 1323, "oldest": 0, "next": 1601,
 "lines": [
   {"data": "<base64>", "width": 143, "wrapped": false},
   ...
 ]}
```

- `lines` are indices `start..start+lines.len()` in oldest-to-newest order,
  ending just before `before`. Page older with `before = start`. `limit`
  defaults to 500 and replies are additionally capped at a byte budget
  (~192KB of line data), so a reply may contain fewer lines than requested
  even when older lines exist — always page by `start`, not by counting.
- Requests older than `oldest` (evicted or never existed) return fewer or
  zero lines; `start == oldest` with an empty remainder means you have
  everything retained.
- Each line's `data` decodes to **self-contained styled text**: printable
  characters and SGR color/attribute sequences only, starting from default
  attributes — no cursor movement, no mode changes. Render it on a fresh
  row (or reset attributes first) of a terminal at least `width` columns
  wide, or strip `ESC[...m` sequences for plain text.
- `width` is the column count the line was laid out at (it varies across
  the session as the wrapper's terminal resizes and views attach at their
  own sizes). `wrapped: true` means the line soft-wraps: it and the next
  record form one logical line, so you may concatenate wrapped runs and
  re-wrap them at your own width; hard lines (`wrapped: false`) wider than
  your screen are yours to crop or wrap as you prefer.

### Input modes and special keys

Sequences that change how a terminal is expected to encode keys — DECCKM
application cursor keys (`CSI ? 1 h/l`), application keypad (`ESC =`/
`ESC >`), bracketed paste (`CSI ? 2004 h/l`), and the xterm mouse modes —
are mirrored unmodified into the snapshot/output byte stream, so the
driver's emulator always knows the current modes. When you eventually send
special keys, encode them according to that state (e.g. Up arrow is
`ESC [ A` normally but `ESC O A` in application-cursor mode), and when
bracketed paste is enabled, wrap inserted text in `ESC [ 200 ~` … `ESC [
201 ~` so apps like editors treat voice input as a paste rather than typed
keystrokes. Protocol version 1 does not model the kitty keyboard protocol
(`CSI > 1 u` etc.): those negotiation sequences are not mirrored, so encode
keys per the classic xterm rules above.

## Terminal data notes

- The byte streams use standard VT100/xterm escape sequences (CSI cursor
  addressing, SGR colors including 256-color and truecolor, alternate
  screen, bracketed paste / mouse / application-key mode toggles, and
  synchronized-update markers `CSI ? 2026 h/l` you may use for flicker-free
  rendering, or ignore).
- Any headless terminal-emulator library that maintains a screen grid works
  (the reference implementation uses Rust's `vt100` crate; xterm.js headless
  works too).
- Device input is supported via the `input` message (see above), gated by
  the two read-only flags.
- Several viewers per session are allowed; they all receive the same
  stream, sized by `size_precedence` (see above). The server's monitor
  connection is separate and always present.

## Liveness and cleanup

- The wrapper removes its socket file on exit. `g2mirror-server` also
  removes, at startup, socket files that are both older than 1 hour and
  owned by a PID that no longer exists.
- `list` only reports sockets whose owning PID is alive; a `connect` can
  still fail if the session exits in between.
- The wrapper drops clients that stop reading for ~5 seconds; keep
  consuming messages promptly while viewing.

## Typical driver flow

```
connect ws://host:8737
→ {"type":"init","version":1,"auth_token":"…","device":"Jim's G2","width":96,"height":24}
← {"type":"init","version":1}
→ {"type":"list"}
← {"type":"sessions","sessions":[…]}          # user picks one
→ {"type":"connect","socket":"84210-_Users_jim_repos_myproj"}
← {"type":"connect","version":1,"pid":84210,…} # attached
→ {"type":"view"}                              # glasses look at terminal
← {"type":"snapshot","data":"…"}               # paint immediately
← {"type":"output","data":"…"}                 # keep applying
…
→ {"type":"unview"}                            # glasses look away
…
→ {"type":"view"}                              # glasses look back
← {"type":"snapshot","data":"…"}
…
← {"type":"exit","status":0}
← {"type":"disconnected","reason":"session closed"}
```
